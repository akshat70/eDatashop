    ...
    <main>
      ...
      <section className="article-body-wrap section-pad section-white">
        <aside className="article-aside">
          <Link href="/blog" className="article-back-link"><ArrowLeft size={15} /> All articles</Link>
          <div className="article-side-block">
            <span className="mono-label accent-label">RECENT POSTS</span>
            <div className="article-side-posts">{recent.map((item) => <Link href={`/blog/${item.slug}`} key={item.slug}><span>{item.category}</span><strong>{item.title}</strong></Link>)}
            </div>
          </div>
          {/* <div className="article-side-block">
